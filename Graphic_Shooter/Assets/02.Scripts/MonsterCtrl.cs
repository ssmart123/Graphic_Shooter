...
public class MonsterCtrl : MonoBehaviour
{
    ...
    void Awake()
    {
        ...
        ////일정한 간격으로 몬스터의 행동 상태를 체크라는 코루틴 함수 실행
    ...
    void ChangeShader()
    {
        if (GameMgr.g_Stone == false && monsterState != MonsterState.die)
        {  // 몬스터가 일반 상태일 때
            animator.speed = 1;
            for (int i = 0; i < m_SMRList.Length; i++)
            {
                ...
                {
                    if (GameMgr.g_DefShader != null &&
                        m_Materials[a].shader != GameMgr.g_DefShader)
                    {
                        m_Materials[a].shader = GameMgr.g_DefShader;
                    }
                }
            }
        } // if (GameMgr.g_Stone == false)
        else if (GameMgr.g_Stone == true && monsterState != MonsterState.die)
        {// 몬스터가 돌이 되었을 때
            animator.speed = 0;
            ...
            for (int i = 0; i < m_SMRList.Length; i++)
            {
                ...
                for (int a = 0; a < m_Materials.Length; a++)
                {
                    if (GameMgr.g_GrayscaleShader != null &&
                        m_Materials[a].shader != GameMgr.g_GrayscaleShader)
                    {
                        m_Materials[a].shader = GameMgr.g_GrayscaleShader;
                    }
                }
        ...
        else if (monsterState == MonsterState.die)  // 몬스터가 죽었을 때
        {
            animator.speed = 1;
            ...
            for (int i = 0; i < m_SMRList.Length; i++)
            {
                ...
                for (int a = 0; a < m_Materials.Length; a++)
                {
                    if (GameMgr.g_DeadShader != null)
                    {
                        m_Materials[a].shader = GameMgr.g_DeadShader;
